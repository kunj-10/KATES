...
def create_model(nb_classes, input_length):
    model = Sequential()
    model.add(
        Conv1D(filters=5, kernel_size=5, padding="valid", input_shape=(input_length, 1))
    )
...
if __name__ == "__main__":
    n_folds = 10
    X, Y, nb_classes, input_length = load_data(
        r"D:\Hackathon\SIH 2025\KATES\output_kmers.csv"
    )
    ...
    for train, test in kfold.split(X, Y):
        ...
        pred, Y_test = train_and_evaluate_model(
            model, X[train], Y[train], X[test], Y[test], nb_classes
        )
